...
const PostsList = ({ currentUserId }) => {
    const { isLoading, error, data: posts, } = useSWR( [postsCacheKey, currentUserId], ([url, userId]) => getPostsByUserId(url, userId) )
    const { isLoading: isLoadingUser, error: userError, data: user } = useSWR( posts?.length ? [usersCacheKey, currentUserId] : null, ([url, userId]) => getUserById(url, userId) )
    let content
    if (currentUserId === 0) {
