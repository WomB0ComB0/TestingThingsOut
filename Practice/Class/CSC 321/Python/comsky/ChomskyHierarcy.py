...
def type1(p, lhs, rhs, l, r):
    ...
    if (lhs == "S") and (
        rhs == "l"
    ):  # this will be applicable only for the first production
        return 1
    if l <= r:
        return 1
...
def type3_RL(p, rhs):
    ...
    if (counter1 == 1) and (
        (rhs[len(rhs)] != "A") or (rhs[len(rhs)] != "B") or (rhs[len(rhs)] != "S")
    ):
        return 3
    else:
        return 2
...
s = int(
    input("Enter the Number of production functions : ")
)  # input number of production functions
