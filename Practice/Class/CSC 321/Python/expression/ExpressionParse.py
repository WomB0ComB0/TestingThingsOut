...
class Tree(Node):
    ...
    def parseInfixNotation(self, expression):
        ...
        while token is not None:
            ...
            if token == Tokenizer.OPENPAREN:
                ...
                if curr_value is None:
                    curr_value = token
                    if (
                        tokenizer.peekToken() is None
                        or tokenizer.peekToken() == Tokenizer.CLOSEPAREN
        ...
        if (
            isinstance(subtree_root, Operation)
            and subtree_root.symbol == "?"
            and subtree_root.right == None
        ):
...
class Variable(Node):
    ...
    def evaluate(self):
        try:
            ...
            raise EvalException(
                "Cannot evaluate expressions that contain uninitialized variables."
            )
...
class Operation(Node):
    ...
    def factor(self):
        ...
        if (
            isinstance(self.left, Operation)
            and type(self.left) == type(self.right)
            and self.left.weight - self.weight == 1
        ):
            # Get grandchildren
            llgc = self.left.left
    ...
    def toInfixNotation(self):
        ...
        # Binary operators
            ...
            raise ValueError(
                "Operators with arity other than 1 or 2 cannot be converted to infix notation"
            )
    ...
    def evaluate(self):
        if self.left is not None and self.right is None:
            ...
            if cvalue >= 0 and cvalue == int(cvalue):
                ...
                raise EvalException(
                    "Cannot compute the factorial of negative numbers or non-integers."
                )
        else:
            raise NodeException("Node does not have enough children.")
