...
class CFG(object):
    ...
    def __init__(
        ...
        if variables is None:
            ...
            raise TypeError(
                "CFG variables must be a iterable, not {}".format(
        ...
        if isinstance(rules, dict):
            ...
            raise TypeError(
                "CFG rules must be a collection or a dict, not {}".format(
                    type(rules).__name__
                )
            )
    ...
    def variables(self, new_variables):
        ...
        if type(new_variables) is not set:
            raise TypeError(
                "CFG variables must be a set, not '{}'".format(
                    type(new_variables).__name__
                )
            )
        ...
        for variable in new_variables:
            if type(variable) is not str:
                raise TypeError(
                    "CFG variables must be strings, not '{}'".format(
        ...
        for variable in new_variables:
            if string_contains_space(variable):
                raise ValueError(
                    "Variables cannot contain white spaces : '{}'".format(variable)
                )

        new_variables_list = list(new_variables)
        for i in range(len(new_variables_list) - 1):
            for j in range(i + 1, len(new_variables_list)):
                contain_each_other, first_str, second_str = strings_contain_each_other(
                    new_variables_list[i], new_variables_list[j]
                )
                if contain_each_other:
                    raise ValueError(
                        "Variables cannot contain each other, '{}' contains '{}'".format(
    ...
    def terminals(self, new_terminals):
        ...
        if type(new_terminals) is not set:
            raise TypeError(
                "CFG terminals must be a set, not '{}'".format(
                    type(new_terminals).__name__
                )
            )
        ...
        for terminal in new_terminals:
            if type(terminal) is not str:
                raise TypeError(
                    "CFG terminals must be strings, not '{}'".format(
        ...
        for terminal in new_terminals:
            if string_contains_space(terminal):
                raise ValueError(
                    "Variables cannot contain white spaces : '{}'".format(terminal)
                )
        ...
        for i in range(len(new_terminals_list) - 1):
            for j in range(i + 1, len(new_terminals_list)):
                contain_each_other, first_str, second_str = strings_contain_each_other(
                    new_terminals_list[i], new_terminals_list[j]
                )
                if contain_each_other:
                    raise ValueError(
                        "Terminals cannot contain each other, '{}' contains '{}'".format(
                            first_str, second_str
                        )
                    )
    ...
    def rules(self, new_rules):
        ...
        if type(new_rules) is not set:
            raise TypeError(
                "CFG rules must be a set, not '{}'".format(type(new_rules).__name__)
            )

        for rule in new_rules:
            if type(rule) is not tuple:
                raise TypeError(
                    "CFG rules must be 2-tuples, not '{}'".format(type(rule).__name__)
                )
            if len(rule) != 2:
            ...
            if string_contains_space(rule[0]) or string_contains_space(rule[1]):
                raise ValueError(
                    "Rule cannot contain white spaces : '{} -> {}'".format(*rule)
                )

        pattern = re.compile(
            "({})+".format("|".join(re_escaped(self.variables | self.terminals)))
        )

        for rule in new_rules:
            if rule[0] not in self.variables:
                raise ValueError(
                    "Unknown Variable '{p0}' in '{p0} -> {p1}'".format(
                        p0=rule[0], p1=rule[1]
                    )
                )
            if not pattern.fullmatch(rule[1]):
                raise ValueError(
                    "Rule must contain combination of variables and terminals : '{} -> {}'".format(
                        *rule
                    )
                )
            if rule[1].count(self.null_character) and rule[1] != self.null_character:
                raise ValueError(
                    "Rule cannot combine null character with variables and terminals : '{} -> {}'".format(
                        *rule
                    )
                )
    ...
    def start_variable(self, new_start_variable):
        ...
        if type(new_start_variable) is not str:
            raise TypeError(
                "CFG start variable must be string, not '{}'".format(
                    type(new_start_variable).__name__
                )
            )
    ...
    def null_character(self, new_null_character):
        ...
        if type(new_null_character) is not str:
            raise TypeError(
                "CFG null character must be string, not '{}'".format(
                    type(new_null_character).__name__
                )
            )
    ...
    def remove_null_rules(self):
        ...
        """
        nullable_vars = {
            rule[0] for rule in self.rules if rule[1] == self.null_character
        }
        ...
        while True:
            is_nullable = re.compile(
                "({})+".format("|".join(re_escaped(nullable_vars)))
            )
            new_nullable_vars = {
                rule[0] for rule in self.rules if is_nullable.fullmatch(rule[1])
            }
        ...
        for rule in self.rules:
            ...
            if splits[0] != rule[1]:
                ...
                def tree_search(node):
                    ...
                    if not children:
                        new_rules.add(
                            (rule[0], ("".join((val[0] for val in node.value))))
                        )
                    else:
    ...
    def _var_none_unit_rules(self, var):
        ...
        return {
            rule[1]
            for rule in self.rules
            if rule[0] == var and not is_unit_rule.fullmatch(rule[1])
        }

    def _var_unit_rules(self, var):
        return {
            rule[1] for rule in self.rules if rule[0] == var
        } - self._var_none_unit_rules(var)
    ...
    def remove_unit_rules(self):
        ...
        non_unit_rules = {
            (var, rule)
            for var in self.variables
            for rule in self._var_none_unit_rules(var)
        }

        for var in self.variables:
            for related_var in related_unit_rules[var]:
                non_unit_rules |= {
                    (var, related_var_non_unit_rule)
                    for related_var_non_unit_rule in self._var_none_unit_rules(
                        related_var
                    )
                }
    ...
    def reduct(self):
        ...
        while True:
            ...
            v1_union_t_pattern = re.compile(
                "({})+".format("|".join(re_escaped(v1_union_t)))
            )
            prev_v1 = copy(v1)
            for var in self.variables:
                if {
                    rule
                    for rule in self.rules
                    if rule[0] == var and v1_union_t_pattern.fullmatch(rule[1])
                }:
                    v1.add(var)
            if prev_v1 == v1:
                break
    ...
    def chamsky(self):
        ...
        def new_var():
            ...
            if len(free_variables) == 0:
                free_variables, last_checked_variable = CFG._generate_var_names(
                    v1, 9, last_checked_variable
                )
                ...
                if not rule_terminals:
                    ...
                    for rule_terminal in rule_terminals:
                        if not terminal_rules.get(rule_terminal, None):
                            ...
                            v1.add(new_variable)
                        old_rule = old_rule.replace(
                            rule_terminal, terminal_rules[rule_terminal]
                        )
        ...
        V = [
            [
                (
                    set()
                    if i != j
                    else {rule[0] for rule in self.rules if rule[1] == string[i]}
                )
        ...
        def Vij(i, j):
            ...
            for k in range(i, j):
                for rule in self.rules:
                    ...
                    if len(rule_variables) == 2:
                        if (
                            rule_variables[0] in V[i][k]
                            and rule_variables[1] in V[k + 1][j]
                        ):
                            V[i][j].add(rule[0])
    ...
    def str_rules(self, *, return_list=False, prepend="", line_splitter="\n"):
        ...
        str_lines = [
            prepend + "{} -> {}".format(var, " | ".join(rules_var[var])) for var in vars
        ]
