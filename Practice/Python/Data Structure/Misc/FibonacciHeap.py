...
import math
class FibonacciTree: # Creating fibonacci tree
    def __init__(self, value):
        self.value = value
        self.child = []
        self.order = 0
    def add_at_end(self, t): # Adding tree at the end of the tree
        self.child.append(t)
        self.order = self.order + 1
class FibonacciHeap: # Creating Fibonacci heap
    def __init__(self):
        self.trees = []
        self.least = None
        self.count = 0
    def insert_node(self, value): # Insert a node
        new_tree = FibonacciTree(value)
        self.trees.append(new_tree)
        if (self.least is None or value < self.least.value):
            self.least = new_tree
        self.count = self.count + 1
    def get_min(self): # Get minimum value
        if self.least is None:
            return None
        return self.least.value
    def extract_min(self): # Extract the minimum value
        smallest = self.least
    ...
    def consolidate(self): # Consolidate the tree
